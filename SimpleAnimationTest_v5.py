...
def Game(numbers):
	...
	while i<30:
		if i==0:
			lst1=game[i]
			lst2=game[i+1]
			lst3=number()[random.randint(0,30)]
		elif i==29:
			break
		else:
			lst1=game[i]
		...
		while j<30:
			if j!=28:
				up=lst3[j]+lst3[j+1]+lst3[j+2]
				low=lst2[j]+lst2[j+1]+lst2[j+2]
				if j!=0:
					middle=lst1[j+1]+lst1[j-1]
				else:
					middle=lst1[j+1]+lst2[random.randint(0,29)]
			else:
				break
			state=up+middle+low
			if state<2:
